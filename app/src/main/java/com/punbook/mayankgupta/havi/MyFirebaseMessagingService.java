...
public class MyFirebaseMessagingService extends FirebaseMessagingService {
    ...
    public void onMessageReceived(RemoteMessage remoteMessage) {
        ...
        if (remoteMessage.getData().size() > 0) {
            ...
            Task task = new Task();
            for (Map.Entry<String, String> dataMap : remoteMessage.getData().entrySet()) {


                switch (dataMap.getKey()) {

                    case "SUMMARY":
                        task.setSummary(dataMap.getValue());
                    default:
                        task.setStartDate(236276323l);
                        task.setExpiryDate(6378263872l);
